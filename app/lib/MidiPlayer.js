...
import Module from './libtimidity.js';
...
export default class MidiPlayer {
    constructor({ eventLogger, logging }) {
        ...
        if (this.audioMethod === 'WebAudioAPI') {
            ...
            this.audioStatus = `audioMethod: WebAudioAPI, sampleRate (Hz): ${
                this.context.sampleRate
            }, audioBufferSize (Byte): ${MIDI_AUDIO_BUFFER_SIZE}`;
    ...
    loadMissingPatch(path, filename) {
        ...
        request.onload = () => {
            if (request.status !== 200) {
                this.emitEvent({
                    event: MIDI_ERROR,
                    message: `Cannot retrieve MIDI patch '${filename}' (status code: ${
                        request.status
                    }).`
                });
                return;
        ...
        request.onload = () => {
            try {
                if (request.status !== 200) {
                    this.emitEvent({
                        event: MIDI_ERROR,
                        message: `Could not retrieve MIDI for '${name}' (status code: ${
                            request.status
                        }).`
                    });
