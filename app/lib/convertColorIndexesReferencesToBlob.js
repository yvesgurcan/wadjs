...
import { TRANSPARENT_PIXEL, COLOR_COUNT_PER_PALETTE } from './constants';
...
export default async function convertColorIndexesReferencesToBlob(
    ...
    if (!palette || palette.length !== COLOR_COUNT_PER_PALETTE) {
        console.error(
            'The palette does not have enough colors to draw images.',
            palette
        );
        return null;
    }
    ...
    try {
        ...
        const {
            canvas: newCanvas,
            context: newContext
        } = createOffscreenCanvas({ height, width });
        ...
        console.error(
            'An error occurred while converting color index references to a blob',
            { error }
        );
        return '';
    }
